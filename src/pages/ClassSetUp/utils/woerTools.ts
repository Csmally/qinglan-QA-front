    ...
    answerList.forEach((answer: any) => {
      ...
      template.groupOptions.forEach((group: any) => {
        ...
        questions.forEach((question: any) => {
          const answerId = answer.answers.find((i: any) => i.questionId === question.id).questionAnswerId;
          const score = question.questionOptions.find((i: any) => i.id === answerId).value;
        ...
        const scoreByGroup = valueGroups.find((i: any) => i.minValue <= groupScore && groupScore <= i.maxValue );
        if (scoreByGroup) {
          scoreReport.push({
...
const wordText = (options: WordTitleOptions) => {
    const { alignment = 'left', spacingAfter = 50, spacingBefore = 0, text, size = 28, color = '#000000', bold = false, shading = false } = options;
...
const wordTableCell = (options: WordTableCellOptions) => {
    ...
    return new TableCell({
        ...
        children: [
            wordText({ alignment: 'center', text, bold, size })
        ]
    })
};

const saveAnalyzeDataAsWord = async (answerList: any, template: any) => {
    console.log('9898结果answerList', answerList)
    console.log('9898结果template', template)
    const doc = new Document({
        sections: answerList.map((answer: any) => ({
            children: [
                    ...
                    rows: [
                        new TableRow({
                            children: [
                                ...
                                wordTableCell({ text: answer.student.sex === '1' ? '男' : '女' }),
                            ]
                        }),
                        new TableRow({
                            children: [
                ...
                wordText({ text: "测验结果", bold: true, spacingAfter: 100, spacingBefore: 200 }),
                new Table({
                    rows: [
                        new TableRow({
                            children: [
                ...
                wordText({ text: "因子得分", bold: true, spacingAfter: 100, spacingBefore: 200 }),
                new Table({
                    rows: [
                        new TableRow({
                            children: [
                        ...
                        ...answer.scoreSummary.map((summary: any) => new TableRow({
                            children: [
                ...
                wordText({ text: "得分解释&&辅导建议", bold: true, spacingAfter: 100, spacingBefore: 200 }),
                ...answer.scoreReport.map((report: any) => {
                    return [
                        wordText({ text: report.title, bold: true, spacingAfter: 100, spacingBefore: 100, shading: true }),
                        wordText({ text: "得分解释", bold: true, spacingAfter: 100, spacingBefore: 100 }),
                        wordText({ text: report.scoreDesc }),
                        wordText({ text: "辅导建议", bold: true, spacingAfter: 100, spacingBefore: 100 }),
                        wordText({ text: report.scoreSug }),
                    ]
                }).flat(),
                wordText({ text: "评语：", bold: true, spacingAfter: 100, spacingBefore: 100 }),
            ]
        }))
    })
    // 将文档打包为 Blob
